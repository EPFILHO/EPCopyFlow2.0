...
### Added
- **`core/latency_tracker.py`** (#111, instrumentação): grava CSV assíncrono em `logs/latency_<timestamp>.csv` com 3 etapas medidas — `T1_recv_trade_event` (latência MQL→Python via `timestamp_mql`), `T2_send_to_slave` (envio do comando ao broker) e `T3_response_from_slave` (round-trip Python→broker→Python). Worker thread dedicado drena a fila e escreve em disco, então as chamadas no hot path são apenas `queue.put_nowait` (microssegundos, não bloqueante). Wired em `main.py` (start/stop), `tcp_message_handler.py` (T1 no recebimento de TRADE_EVENT do master) e `tcp_router.send_command_to_broker` (T2/T3 cobrindo todo round-trip). **Objetivo:** descobrir empiricamente onde estão os 10s+ de lag durante Alt+Tab — se em `T1_recv_trade_event` o gargalo é o MT5 do master throttlado pelo Windows; se em `T3 - T2` é o slave throttlado; se entre eles, é Python. Sem isso, qualquer fix é tiro no escuro.
...
- **`CopyTradeManager`**: agora construído dentro do bootstrap do motor (sqlite connection nasce na thread do motor). `sqlite3.connect(check_same_thread=False)` + `threading.Lock` (`_db_lock`) habilitam as 2 leituras síncronas que ainda são feitas pela main thread (`get_trade_history`, `get_today_stats`). Solução **intermediária** documentada no código — PR 3 trocará por modelo signal-based.
